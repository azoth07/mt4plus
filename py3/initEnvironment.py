...
def CreateSymbols(sym_name,sym_group,sym_Color):
    ...
    with open(symbols_raw_path,"wb") as rawFile:
...
#             为了防止过长
            ...
            if sgu == "5":
                sgu_content = "\x05"
            sgu_content= bytes(sgu_content,'UTF-8')
            ...
            green = int(color[3:6])
            blue = int(color[6:9])
            rawFile.write(supplementZero(b"\x02",4))
            rawFile.write(supplementZero(b"\x02",4))
            rawFile.write(struct.pack("3B",red,green,blue))
            rawFile.write(b"\x00")
            rawFile.write(b"\x01")
            rawFile.write(b"\x00"*3)
            rawFile.write(supplementZero(b"\x51",1816))
...
def create_groups(grp_name,grp_Desc):
    ...
    with open(symgroups_raw_path,"wb") as  groupFile:
        symgroups = 2560
        for i in range(1,len(grp_name)+1):
            coreid = grp_name[i-1].strip()
            if isinstance(coreid ,str):
