...
def get_price_day_tx(code, end_date='', count=10, frequency='1d'):     #日线获取  
    ...
    df=pd.DataFrame(buf,columns=['day','open','close','high','low','volume'],dtype='float')     
...
def get_price_sina(code, end_date='', count=10, frequency='60m'):    #新浪全周期获取函数    
    ...
    #df.day=pd.to_datetime(df.day);    df.set_index(['day'], inplace=True);     df.index.name='11'           #处理索引                 
